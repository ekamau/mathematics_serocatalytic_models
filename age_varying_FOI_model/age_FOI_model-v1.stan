...
data{
  ...
  int n_pos[n_obs]; // number seropositive
  int n_total[n_obs]; // number tested
  int<lower=0> age_max;
  int chunks[age_max];
  int ages[n_obs];
...
parameters{
  row_vector[n_chunks] log_foi;
  real<lower=0> sigma[is_random_walk ? 1 : 0]; 
  real<lower=0> nu[is_random_walk ? 1 : 0]; 
  real<lower=0> seroreversion_rate[include_seroreversion ? 1 : 0]; 
...
model{
  ...
  } else if(foi_prior_choice == 6) { // cauchy
