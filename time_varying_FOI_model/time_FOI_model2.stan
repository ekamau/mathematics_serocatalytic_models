// this model assumes a constant or fixed seroreversion rate; part of the user-input in the data.
...
functions {
  ...
  real solve_ode_one_year(real prob_previous, real foi, real seroreversion_rate) {
    real e_upper = exp(foi + seroreversion_rate);
    real e_lower = exp(-(foi + seroreversion_rate));
    
    return e_lower * ((prob_previous - 1 + e_upper) * foi + prob_previous * seroreversion_rate) / (foi + seroreversion_rate);
  }
  
  real prob_infected_seroreversion_single_age(vector fois_per_year_for_one_age_group, real seroreversion_rate) {
    
    int n_iter = num_elements(fois_per_year_for_one_age_group);
    real prob_infected = 0;
    
    for(i in 1:n_iter) {
      prob_infected = solve_ode_one_year(prob_infected, fois_per_year_for_one_age_group[i], seroreversion_rate);
    }
